...
export default class EditorSelectionManipulator implements EditorSelection {
	...
	equals(sel: EditorSelection): boolean {
	...
	moveChars(anchor: number, head?: number): this {
		this.anchor = new EditorPositionManipulator(
			this.editor.offsetToPos(this.editor.posToOffset(this.anchor) + anchor),
			this.editor,
		);
		this.head = new EditorPositionManipulator(
			this.editor.offsetToPos(this.editor.posToOffset(this.head) + (head ?? anchor)),
			this.editor,
		);
		return this;
	}
	...
	static listSelections(editor: Editor): EditorSelectionManipulator[] {
